...
- **Integration tests are env-gated.** `SORTIE_JIRA_TEST=1` for Jira, `SORTIE_GITHUB_TEST=1` for GitHub adapter integration tests, `SORTIE_GITHUB_E2E=1` for GitHub E2E orchestrator tests (also requires `SORTIE_GITHUB_TOKEN` and `SORTIE_GITHUB_PROJECT`), `SORTIE_LINEAR_TEST=1` for Linear (also requires `SORTIE_LINEAR_API_KEY` and `SORTIE_LINEAR_TEAM_KEY`; optional `SORTIE_LINEAR_TEAM_NAME`, `SORTIE_LINEAR_TEST_ISSUE_IDENTIFIER`, `SORTIE_LINEAR_ACTIVE_STATES`, `SORTIE_LINEAR_ENDPOINT`, `SORTIE_LINEAR_USE_BEARER`), `SORTIE_CLAUDE_TEST=1` for Claude Code, `SORTIE_COPILOT_TEST=1` for Copilot. Without these vars, integration tests must skip cleanly — never fail.
